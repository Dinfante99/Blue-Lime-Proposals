...
Auth:
  ...
  in the environment. The service refuses to start without it set so auth
  cannot be silently disabled in production.
"""
...
def _check_api_key(provided: str | None) -> None:
    ...
    If PROPOSAL_API_KEY is unset, raise RuntimeError so the request fails
    loudly — auth must always be configured in production.
    """
    if not API_KEY:
