...
def _make_cover_drawer(data):
    ...
    def draw_cover(canv, doc):
        ...
        # Split long association names across two cover lines for readability.
        # We try several common splits in priority order; if none match, the
        # whole name renders on one line (still works at 20pt for ~50 chars).
        name = client["name"]
        SPLIT_MARKERS = [
            " Owners Association",
        ...
        for marker in SPLIT_MARKERS:
            if marker in name:
                short, suffix = name.split(marker, 1)
                canv.drawCentredString(cx, PAGE_H / 2 - 68, short.strip())
                canv.setFont("Helvetica-Bold", 16)
                canv.drawCentredString(cx, PAGE_H / 2 - 90, marker.strip() + suffix)
                split_done = True
                break
        if not split_done:
        ...
        canv.drawRightString(box_x + box_w - 20, box_y + 14,
                             f"Blue Lime Insurance Group  \u00b7  www.bluelimeins.com  \u00b7  {am['email']}")
    return draw_cover
...
def _make_interior_drawer(data):
    def draw_interior(canv, doc):
        ...
        am_email = data["account_manager"]["email"]
        canv.setFont("Helvetica", 8.5)
        canv.setFillColor(BL_MUTED)
        canv.drawString(LEFT, BOTTOM - 0.42 * inch,
                        f"Blue Lime Insurance Group  \u00b7  San Antonio, TX  \u00b7  {am_email}")
...
def _summary_page(data):
    ...
    cols = [
        ...
        [p("Why Blue Lime", "H3"),
         p("We exclusively serve community associations &mdash; not a "
           "side line of business. As an independent brokerage, we shop "
           "carriers selected for HOA-specific coverages, conduct a "
           "mid-term continuation analysis to keep your program aligned "
           "with changes in the community, and assign every policy a "
           "dedicated claims advocate who works directly with carriers "
           "to protect your reserves and limit renewal-rate impact.", "Small")],
        [p("Next Steps", "H3"),
         p(f"Review the coverage detail and premium comparison. When you're "
           f"ready to bind, sign the authorization page and return it to "
           f"<b>{esc(am['email'])}</b>. We'll handle the rest.", "Small")],
    ]
...
def _coverage_pages(data):
    ...
    for cov in data["coverages"]:
        ...
        # When a coverage is not included, suppress the detail panels — they
        # otherwise show stale template defaults that contradict the "Not
        # Included" status badge. Show a single placeholder row instead.
        if cov.get("not_included"):
            panel_rows = [[
                ("Coverage", "Not included on this proposal"),
                ("", ""),
                ("", ""),
                ("", ""),
            ]]
        else:
            panel_rows = cov["panel_rows"]
...
def _authorization_page(data):
    ...
    story.append(Spacer(1, 10))
    story.append(p(f"Please return the completed authorization to your account manager at "
                   f"<b>{esc(data['account_manager']['email'])}</b>.", "Small"))
    return story
...
def _disclosures_page():
    ...
    paragraphs = [
        ...
        "Premiums charged by insurance companies are determined by the amount of exposure or risk of loss. These exposures were based on the information provided to our agency during the application process. Additional exposures often arise during the policy period &mdash; including new structures, property, or owners/occupants. To ensure an appropriate amount of coverage remains in place, please inform us of any changes in the community. Changes to deductibles and/or limits of liability can affect the total premium.",
        "Blue Lime is compensated through carrier commissions and an agency service fee. The agency fee funds the work that sets us apart from a transactional brokerage: a dedicated claims advocate who reports losses on your behalf and negotiates directly with carriers to limit renewal-rate impact, a mid-term continuation analysis to keep coverage aligned with changes in your community, annual Statement of Values reviews, and unlimited access to your account manager throughout the policy term. Carrier commissions are set by each insurer as a percentage of premium and vary by company. From time to time, intermediary wholesale brokers may be used to access specific markets &mdash; including non-admitted (surplus lines) carriers &mdash; in which case our compensation may come from a portion of the intermediary's commission. The agency fee is not set by law and may be in lieu of or in addition to any commissions received.",
