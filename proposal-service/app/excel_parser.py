...
The parser is tolerant of:
  ...
  - Placeholder values like "XXXX", "X% Deductible", "N/A" → treated as absent
  - Trailing whitespace and minor capitalization variation
  - Decimal-encoded percentages (e.g., 0.05 in a deductible cell → "5%")
...
# Regex matching template placeholder strings like "X% Deductible", "X%",
# "XXX", "XX%", etc. — used for cells where the team types a literal "X" or
# "XX" to mean "not applicable / fill in later."
_PLACEHOLDER_RE = re.compile(r"^x+\s*(%|\s|deductible).*$|^x+$", re.IGNORECASE)
...
def _deductible_str(v: Any, default: str = "Standard") -> str:
    """Format a deductible cell. Detects percentages (values < 1 are treated
    as percentages, e.g., 0.05 → '5%') and formats round dollars otherwise.
    Strings are passed through unchanged."""
    if _is_placeholder(v):
        return default
    if isinstance(v, (int, float)):
        if 0 < v < 1:
            # Looks like a percentage stored as a decimal
            pct = v * 100
...
def _limit_str(v: Any, default: str = "Not Included") -> str:
    ...
    Detects percentage-like decimals (0 < v < 1) for deductible-style cells."""
    if _is_placeholder(v):
        return default
    if isinstance(v, (int, float)):
        if 0 < v < 1:
...
def _parse_client(wb) -> dict:
    ...
    # short_name = a header-friendly version of the association name. Used in
    # the page-header banner where space is tight. We strip common suffixes
    # in order of specificity to keep the most identifying part of the name.
    short_name = name
    SUFFIX_PATTERNS = [
        r"\s+Owners Association.*$",
        r"\s+Homeowner'?s Association.*$",
        r"\s+Residential Community.*$",
        r"\s+Community Association.*$",
        r"\s+Property Owners.*$",
        r"\s+Association,?\s*Inc\.?\s*$",
        r",?\s*Inc\.?\s*$",
    ]
    for pat in SUFFIX_PATTERNS:
        candidate = re.sub(pat, "", short_name, flags=re.IGNORECASE).strip()
        if candidate and len(candidate) < len(short_name):
            short_name = candidate
            break  # take the first matching pattern (most specific)
    if not short_name:
        short_name = name
...
def _parse_premium_comparison(wb) -> dict:
    ...
    for label, exp_c, prop_c in rows:
        ...
        # auxiliary (Cyber, Crime, W/H Buyback). This keeps the table tight.
...
def _valuation_basis(wb) -> tuple[str, str]:
    """Detect whether property is insured on RCV or ACV basis.

    Looks at two locations in priority order:
      1. Cell B26 of the Summary tab — explicit override. Team can type "RCV",
         "ACV", "Replacement Cost", "Actual Cash Value", or similar.
      2. Cell A23 of the Summary tab — the prose narrative the team writes
         describing the proposed coverage. Scanned for keywords.

    Returns (short_label, long_label) where short_label is "RCV", "ACV", or
    "" (unknown), and long_label is "replacement cost (RCV)",
    "actual cash value (ACV)", or "" (unknown).
    """
    s = wb["Summary"]
    ...
    def _classify(text: str) -> tuple[str, str] | None:
        if not text:
            return None
        t = text.lower().strip()
        # Order matters: check ACV first since "actual cash value" doesn't
        # contain the word "replacement"
        if "actual cash value" in t or t == "acv" or " acv" in t or "(acv)" in t:
            return ("ACV", "actual cash value (ACV)")
        if "replacement cost" in t or t == "rcv" or " rcv" in t or "(rcv)" in t or t == "rc":
            return ("RCV", "replacement cost (RCV)")
        return None

    # Priority 1: explicit override in B26
    explicit = _cell(s, "B26")
    if explicit and not _is_placeholder(explicit):
        result = _classify(str(explicit))
        if result:
            return result

    # Priority 2: prose narrative in A23
    narrative = _cell(s, "A23")
    if narrative:
        result = _classify(str(narrative))
        if result:
            return result

    # Unknown — return empty strings, generator will use a generic phrase
    return ("", "")
...
def _parse_coverages(wb) -> list[dict]:
    ...
    # Valuation basis (RCV vs ACV) — read from Summary tab. If unknown, the
    # description falls back to generic phrasing and the panel shows a dash.
    val_short, val_long = _valuation_basis(wb)

    if val_long:
        valuation_phrase = f"This proposal covers property on a {val_long} basis."
        valuation_panel_value = val_short
    else:
        valuation_phrase = "Valuation basis is set per the policy form."
        valuation_panel_value = "See policy"
    ...
    coverages.append({
        ...
        "description": (
            ...
            f"perils. {valuation_phrase} Flood and Earthquake are excluded "
        ...
        "panel_rows": [
            ...
            [
                ("AOP Deductible",       _deductible_str(_cell(s, "H26"))),
                ("Wind / Hail Ded.",     _deductible_str(_cell(s, "I26"))),
                ("Valuation Basis",      valuation_panel_value),
    ...
    # Special handling: if not included, hide entirely from the proposal
    # rather than showing a "Not Included" badge (per Blue Lime preference —
    # the buyback is an optional add-on, not an expected coverage line).
    wh_carrier = _cell(s, "A36")
    wh_not_included = _is_placeholder(wh_carrier)
    if not wh_not_included:
        coverages.append({
    ...
    coverages.append({
        ...
        "panel_rows": [[
            ...
            ("Cyber Liability", _limit_str(_cell(s, "I64"), "Not Included")),
            ("Retention",      _limit_str(_cell(s, "J64"), "Standard")),
        ]],
    })
...
def _parse_sov(wb) -> dict:
    ...
    The current Excel templates only have totals on the Summary tab; the SOV
    tab itself contains a long itemized list. To stay tolerant, we fall back
    to pulling values from the Summary tab's Property panel (rows 25-26)
    when the SOV tab is sparse.
    """
    summary = wb["Summary"]
    ...
    # Outdoor property breakdown — present the rolled-up Outdoor Property line.
    # Per-item allocations aren't reliably in known cells across templates.
    outdoor_items = []
    if outdoor_value > 0:
